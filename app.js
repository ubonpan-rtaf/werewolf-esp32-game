...
function toggleBGM() {
    playSound('click');
    if(isBgmPlaying){ bgm.pause(); isBgmPlaying=false; showToast("ปิดเพลงประกอบแล้ว"); } 
    else { bgm.play(); isBgmPlaying=true; showToast("เปิดเพลงประกอบแล้ว"); }
}
...
db.ref("werewolf_rooms").on("value", snap => {
    ...
    if(snap.exists()) {
        snap.forEach(r => {
            if(r.val().system && r.val().system.gameState !== "lobby") return; // โชว์เฉพาะห้องที่รอคน
            let opt = document.createElement("option"); opt.value = r.key; opt.innerText = `🎮 ห้อง: [ ${r.key} ]`;
...
async function joinRoom() {
    ...
    if(!roomInput || roomInput === "NEW") {
        ...
        if(!roomSnap.exists()) return showToast("ไม่พบรหัสห้องนี้ หรือห้องกำลังเล่นอยู่!", "error");
    ...
    document.getElementById("game-screen").style.display = "flex"; // ใช้ Flex
    document.getElementById("room-display").innerText = "ROOM: " + currentRoom;
    if(isHost) { document.getElementById("force-next-btn").style.display = "block"; document.getElementById("force-next-btn").innerText = "👑 [Host] เริ่มเกม (แจกบท)"; }
...
function listenToGame() {
    ...
    db.ref(`werewolf_rooms/${currentRoom}/system`).on("value", snap => {
        const sys = snap.val(); if(!sys) { alert("ห้องถูกปิดแล้ว กลับสู่หน้าหลัก"); window.location.reload(); return; }
...
async function forceNextPhase() {
    if(!isHost || isProcessingPhase) return;
    isProcessingPhase = true; playSound('click');
    let next = "lobby"; let duration = 0;
    
    if(lastState === "lobby" || lastState === "wolf_win" || lastState === "villager_win") { 
        await assignRoles(); next = "role_reveal"; duration = 12000; 
    ...
    setTimeout(() => { isProcessingPhase = false; }, 1000);
}
...
async function assignRoles() {
    ...
    await db.ref(`werewolf_rooms/${currentRoom}/votes`).remove(); await db.ref(`werewolf_rooms/${currentRoom}/wolf_kills`).remove(); await db.ref(`werewolf_rooms/${currentRoom}/guardian_shields`).remove(); await db.ref(`werewolf_rooms/${currentRoom}/pranks`).remove();
}
...
async function checkWin() {
    ...
    if(w === 0 && v > 0) { document.getElementById("force-next-btn").innerText = "👑 เริ่มเกมใหม่"; return "villager_win"; }
    if(w >= v && w > 0) { document.getElementById("force-next-btn").innerText = "👑 เริ่มเกมใหม่"; return "wolf_win"; }
    return "night_phase";
}
...
function updateGameUI(state) {
    ...
    if(state==="role_reveal") { ind.innerText="สถานะ: แจกบทบาท"; txt.innerText="ชะตากรรมของท่านถูกกำหนดแล้ว จงจดจำบทบาทของตนไว้ให้ดี..."; }
        ...
        ind.innerText="☀️ แสงอรุณรุ่งสว่าง"; txt.innerText="รุ่งอรุณมาถึง สำรวจความเสียหายและใช้สกิลเค้นความจริง!"; 
        showKills(voteArea); if(!isDead) loadActions(actionArea, "prank"); 
    }
    else if(state==="voting_phase") { 
        ind.innerText="⚖️ การพิพากษาแห่งสภา"; txt.innerText="เวลาแห่งการโหวตประหารมาถึงแล้ว!"; 
        if(!isDead) loadActions(actionArea, "vote"); 
    }
        ...
        if((mode !== 'protect' && k !== myPlayerId) || mode === 'protect') {
            if(p[k].isAlive) {
                ...
                btn.onclick = () => {
                    ...
                    else if(mode==="prank") { db.ref(`werewolf_rooms/${currentRoom}/pranks/${k}`).set("เพื่อนร่วมทีม"); showToast(`ส่งคำท้าไปหา ${p[k].name} แล้ว!`, "warning"); }
...
// Logic: Check Kills and Shields (Master Self-Update)
...
async function showVotes(container) {
    ...
    if(isTie || !target) {
        container.innerHTML = `<div class='vote-result-box' style='border-color:#ffd700;'>⚖️ มติสภาเสียงเท่ากัน... ไม่มีผู้ใดถูกขับไล่!</div>`;
        return;
    }

    const pSnap = await db.ref(`werewolf_rooms/${currentRoom}/players/${target}`).once("value"); const p = pSnap.val();
    container.innerHTML = `<div class='vote-result-box'>มติสภา (${max} เสียง) ขับไล่ <div class='vote-winner'>${p.name}</div><hr style='border:1px solid #555; margin:8px 0;'>ความจริง: <b>${p.name} คือ ${p.role==='wolf'?'<span style="color:#ff4757">หมาป่า! 🐺</span>':'<span style="color:#2ed573">ผู้บริสุทธิ์... 🧑</span>'}</b></div>`;
    
    if(target === myPlayerId) {
        db.ref(`werewolf_rooms/${currentRoom}/players/${target}/isAlive`).set(false); wasInterrogated = false;
    } else if(wasInterrogated && myRole !== 'wolf') {
        myTrust+=5; db.ref(`werewolf_rooms/${currentRoom}/players/${myPlayerId}/trust`).set(myTrust); showToast("✨ รอดพ้นข้อครหา Trust +5%", "mystic"); wasInterrogated = false;
    }
}
